...
def fixer(y, z_bool, ma=5):
    ...
    spikes = np.reshape(spikes,(len(spikes[0]),)) # must reshape from (1,len) to (len,) for the cycling through i in the for loop to work
    for i in spikes: # uses the locations of spikes as i values
        w = np.arange(max(0, i - ma), min(n, i + 1 + ma)) # sets a range around i, if close to array boundaries, limits range to boundary, +1 becuase the way python references does not include top value
        w = w[z_bool[w] == 0] # returns the non-peak locations from the range
        y_out[i] = np.mean(y[w]) # replaces the peak with a mean of sarounding non-peak data
    return y_out
