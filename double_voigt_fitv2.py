...
def _2Voigt(x, ampG1, cen1, sigmaG1, ampL1, widL1,
            ampG2, cen2, sigmaG2, ampL2, widL2):
    return (ampG1*(1/(sigmaG1*(np.sqrt(2*np.pi))))*(np.exp(-((x-cen1)**2)/(2*sigmaG1**2)))) +\
            ((ampL1*widL1**2/((x-cen1)**2+widL1**2)) ) +\
                (ampG2*(1/(sigmaG2*(np.sqrt(2*np.pi))))*(np.exp(-((x-cen2)**2)/(2*sigmaG2**2)))) +\
                    ((ampL2*widL2**2/((x-cen2)**2+widL2**2)) )
...
for i in material:
    ...
    pars_1 = popt_2voigt[0:5]    
    pars_2 = popt_2voigt[5:10]
    ...
    # The weighting is calculated by taking the ratio of the Lorentz and Gauss
    # prefactors, i.e. the intensity magnitudes.
